...
class NoteModel
{
    ...
    public static function createNote($note_text)
    {
        if (!$note_text || strlen($note_text) == 0) {
            Session::add('feedback_negative', _('FEEDBACK_NOTE_CREATION_FAILED'));
            return false;
        }
        ...
        public static function saveEmail($email)
    {
        if (!$email || strlen($email) == 0) {
            Session::add('feedback_negative', 'No email address supplied');
            return false;
        }
        
        $email = strip_tags($email);
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        Session::add('feedback_negative', 'This email address looks suspicious. Care to try to retype it, perhaps?');
        return false;
        }

        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "INSERT INTO emails (email, ip, timestamp) VALUES (:email, :ip, :timestamp)";
        $query = $database->prepare($sql);
        $query->execute(array(':email' => $email, ':ip' => $_SERVER['REMOTE_ADDR'], ':timestamp' => time()));

        if ($query->rowCount() == 1) {
        Session::add('feedback_positive', 'Thank you! We will let you know as soon as we\'re ready');
            return true;
        }
    ...
    public static function updateNote($note_id, $note_text)
    {
        ...
        Session::add('feedback_negative', _('FEEDBACK_NOTE_EDITING_FAILED'));
        return false;
    }
    ...
    public static function deleteNote($note_id)
    {
        ...
        // default return
        Session::add('feedback_negative', _('FEEDBACK_NOTE_DELETION_FAILED'));
        return false;
    }
}
